...
export async function submitWithOfflineSupport(
  ...
  const checkNetwork = async (): Promise<boolean> => {
    try {
      const API_BASE_URL =
        process.env.EXPO_PUBLIC_API_BASE_URL || "https://dev.greenlegacy.ng";
  ...
  if (isOnline) {
    ...
    try {
      if (__DEV__) {
        console.log(
          `[SubmitWithOfflineSupport] Online - submitting ${type} immediately`,
        );
      }
      ...
      syncQueuedSubmissions().catch((error) => {
        if (__DEV__) {
          console.error(
            "[SubmitWithOfflineSupport] Error syncing queued submissions:",
            error,
          );
        }
      });
      ...
      if (__DEV__) {
        console.log(
          `[SubmitWithOfflineSupport] Submission failed, queuing ${type}:`,
          errorMessage,
        );
      }
    ...
    try {
      ...
      return {
        ...
        queued: true,
        message:
          "Device is offline. Submission has been queued and will sync when online.",
      };
    } catch (error: any) {
